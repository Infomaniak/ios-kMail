    ...
    public struct MailThreadEntityQuery: EntityQuery {
        ...
        public func entities(for identifiers: [MailThreadEntity.ID]) async throws -> [MailThreadEntity] {
            ...
            return mailboxes.flatMap { mailbox -> [MailThreadEntity] in
                ...
                return threads.map { MailThreadEntity(thread: $0, mailbox: mailbox) }
            }
        }
    }
}
...
@available(iOS 27.0, *)
extension MailThreadEntity {
    init(thread: Thread, mailbox: Mailbox) {
        id = thread.uid
        title = thread.formattedSubject
        messages = thread.messages.map { MailMessageEntity(message: $0, mailbox: mailbox) }
    }
}
