...
extension RandomAccessCollection where Element == Message {
    ...
    public func lastMessageToExecuteAction(currentMailboxEmail: String,
        ...
        for message in self {
            if message.canExecuteAction(featureAvailableProvider: featureAvailableProvider) {
...
public class ActionsManager: ObservableObject {
    ...
    public func performAction(target messages: [Message], action: Action,
        ...
        case .showEuriaActions:
            ...
            Task { @MainActor in
                origin.actionState.messageToProcessWithEuria?.wrappedValue = message
            }
        case .replyWithEuria:
    ...
    private func performReplyWithEuria(messages: [Message], origin: ActionOrigin) {
        ...
        if NoReplyAlert.verifySenders(
            ...
            origin.actionState.nearestNoReplyAlert?.wrappedValue = NoReplyAlertState {
                origin.actionState.nearestAIWriterReplyPanel?
            ...
            origin.actionState.nearestAIWriterReplyPanel?.wrappedValue = AIWriterReplyPanelState(
                replyingMessageUid: replyingMessage.uid,
                replyMode: replyMode
            )
        }
